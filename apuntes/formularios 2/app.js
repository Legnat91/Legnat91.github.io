document.addEventListener("DOMContentLoaded", () => {
  ...
  // Añade esta función dentro de tu DOMContentLoaded
  async function enviarFormularioServidor(datos) {
    try {
      ...
      alert("¡Datos enviados con éxito al servidor!");

    } catch (error) {
      console.error("Error en la petición POST:", error);
      mensaje.innerHTML = "<p class='aviso'>Error al conectar con el servidor</p>";
    }
  }

  // Añade esta función para cargar algo al inicio
  async function cargarPaisesAPI() {
    try {
      // Usamos una API de prueba de usuarios para simular nombres de "países" o "usuarios"
      ...
      // Limpiamos el select actual (opcional)
      // pais.innerHTML = '<option value="" selected disabled>--Seleccione--</option>';

      usuarios.forEach(user => {
        const option = document.createElement("option");
        option.value = user.username.toLowerCase();
  ...
  formulario.addEventListener("submit", (e) => {
    ...
    if (control) {
      ...
      // LLAMADA AL POST
      enviarFormularioServidor(datos);
      formulario.reset();
    };
  ...
  cargarFormulario();
  cargarPaisesAPI(); // Llamada al GET para rellenar el select
